...
void rc4::cipher(const std::string plaintext, const std::string key, const bool isHex)
{
    try
    {
        if(plaintext!="")
        {
            ...
            throw CryptExceptions("The plaintext must be not null");
    }
...
void rc4::decipher(const std::string ciphered, const std::string key)
{
    try
    {
        if(ciphered!="")
        {
            ...
            throw CryptExceptions("The ciphered messge must not be null");
    }
    catch (CryptExceptions e)
    {
        e.alert();
...
std::string rc4::hex_to_string(const std::string& input)
{
    ...
    try
    {
        if (len & 1)
        {
            throw CryptExceptions("odd length");
            output.clear();
        }
        else
        {
            ...
            for (size_t i = 0; i < len; i += 2)
            {
                ...
                if (*p != a)
                    throw CryptExceptions("not a hex digit");
